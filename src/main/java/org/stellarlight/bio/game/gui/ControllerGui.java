...
public class ControllerGui extends GuiContainer {
    ...
    public ControllerGui(Block block) {
        super(new Container() {
            @ParametersAreNonnullByDefault
            @Override
            public boolean canInteractWith(EntityPlayer playerIn) {
                return block instanceof Controller;
            }
        });
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
        ...
        drawString(fontRenderer, String.format("已存储 Bio: %d/%d", 0, controller.getMaxBioCapacity()), baseX - width + 10, baseY - height + fontRenderer.FONT_HEIGHT + 2, 0xffffffff);
        ...
        drawString(fontRenderer, String.format("已绑定中继器: %d/%d", 0, controller.getMaxBindableRelay()), baseX - width + 10, baseY - height + (fontRenderer.FONT_HEIGHT * 3) + 2, 0xffffffff);
    }
}
